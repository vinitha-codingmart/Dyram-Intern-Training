...
function join(rtc, option) {
  ...
  rtc.client.init(
    ...
    function() {
       ...
       **/
      let channel = localStorage.getItem("streamName");
      ...
      rtc.client.join(
        ...
        null,
        function(uid) {
          Toast.notice("join channel: " + channel + " success, uid: " + uid);
...
function getStats(rtc) {
  setInterval(() => {
    rtc.client.getSessionStats(stats => {
      document.getElementById(
        "stats-div"
      ).innerHTML = `Current Session Duration: ${stats.Duration}<br/>Current Session UserCount: ${stats.UserCount}<br/>Current Session SendBytes: ${stats.SendBytes}<br/>Current Session RecvBytes: ${stats.RecvBytes}<br/>Current Session SendBitrate: ${stats.SendBitrate}<br/>Current Session RecvBitrate: ${stats.RecvBitrate}`;
    });
  }, 1000);
}
